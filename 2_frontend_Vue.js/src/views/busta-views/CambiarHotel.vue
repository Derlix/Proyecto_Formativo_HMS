...
const changeHotelNew = async () => {
    ...
    try {
        ...
        alert("¡Código de hotel actualizado con éxito!")
        router.push("/") // Redirigir a la página de inicio después de cambiar el código de hotel
    } catch (err) {
        errorMessage.value = err.response?.data?.detail || "Error actualizando el código de hotel. Verifique el código."
        setTimeout(() => {
                errorMessage.value = null;
            }, 3000);
    }
